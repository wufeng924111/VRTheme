...
UIKIT_EXTERN NSString * _Nonnull const kVRThemeDefault;

typedef NSString VRThemeName;

SEL _Nullable getSelectorWithPattern(const char * _Nullable prefix, const char * _Nullable key, const char * _Nullable suffix);

typedef NS_ENUM(NSInteger, VRThemeType) {
    VRThemeTypeMainBundle,
    VRThemeTypeSandbox
};

/** Download task status. */
typedef NS_ENUM(NSInteger, VRThemeDownloadTaskStatus) {
    VRThemeDownloadTaskStatusNew = 1, // New task
    VRThemeDownloadTaskStatusAlreadyExist, // Alreading exist task
    VRThemeDownloadTaskStatusDownloading, // Downloading task
    VRThemeDownloadTaskStatusSuspending // Suspending task
};

@class VRThemeManager;

@protocol VRThemeDownloadProtocol <NSObject>

@property (copy, nonatomic, nonnull) NSString * remoteURL;

@property (copy, nonatomic, nullable) VRThemeName * sakuraName;

@end

@protocol VRThemeDownloadDelegate <NSObject>

@optional

- (void)sakuraManagerDownload:(VRThemeManager *_Nullable)manager
...
- (void)sakuraManagerDownload:(VRThemeManager *_Nullable)manager
                 downloadTask:(NSURLSessionDownloadTask *_Nullable)downloadTask
                  sakuraInfos:(id<VRThemeDownloadProtocol>_Nullable)infos
...
@interface VRThemeManager : NSObject
...
+ (BOOL)shiftSakuraWithName:(VRThemeName *_Nullable)name type:(VRThemeType)type;

+ (VRThemeType)getSakuraCurrentType;

+ (VRThemeName *_Nullable)getSakuraCurrentName;

#pragma mark - Manual Sakura

+ (NSArray<VRThemeName *> *_Nullable)getLocalSakuraNames;

+ (void)registerLocalSakuraWithNames:(NSArray<VRThemeName *> *_Nullable)names;

//+ (void)addRemoteSakuraWithName:(VRThemeName *_Nullable)name resourceURL:(NSURL *_Nullable)url;

@end

#pragma mark - TXSerialization

@interface VRThemeManager(TXSerialization)
...
@interface VRThemeManager(TXTool)
...
typedef void(^VRThemeDownloadProgressHandler)(int64_t bytesWritten, int64_t totalBytesWritten, int64_t totalBytesExpectedToWrite);
/** Download completed block handler */
typedef void(^VRThemeErrorHandler)(NSError * _Nullable error);
/** Unzip download file progress block handler */
typedef void(^VRThemeUnzipProgressHandler)(unsigned long long loaded, unsigned long long total);
/** Unzip download file completed handler */
typedef void(^VRThemeDownloadCompletedHandler)(id<VRThemeDownloadProtocol>_Nullable infos, NSURL * _Nullable location);
/** Custom download operation block handler */
typedef void(^VRThemeGeneratePathSuccessHandler)(NSString *_Nullable toFilePath, NSString *_Nullable sakuraPath, VRThemeName *_Nullable sakuraName);

@interface VRThemeManager(TXDownload)<NSURLSessionDownloadDelegate>

/** return list of all sakura that you have downloaded. */
+ (NSArray<VRThemeName *> *_Nullable)tx_getSakurasList;
...
- (NSURLSessionDownloadTask *_Nullable)tx_sakuraDownloadWithInfos:(id<VRThemeDownloadProtocol> _Nullable)infos
                                                          delegate:(id<VRThemeDownloadDelegate> _Nullable)delegate;

- (NSURLSessionDownloadTask *_Nullable)tx_sakuraDownloadWithInfos:(id<VRThemeDownloadProtocol> _Nullable)infos
                                           downloadProgressHandler:(VRThemeDownloadProgressHandler _Nullable )downloadProgressHandler
                                              downloadErrorHandler:(VRThemeErrorHandler _Nullable)downloadErrorHandler
                                              unzipProgressHandler:(VRThemeUnzipProgressHandler _Nullable)unzipProgressHandler
                                                  completedHandler:(VRThemeDownloadCompletedHandler _Nullable)completedHandler;
...
/**
 ...
 you may need to use this method to generate some paths that make VRThemeKit can control it.
 ...
 */
- (void)tx_generatePathWithInfos:(id<VRThemeDownloadProtocol>_Nullable)infos
            downloadFileLocalURL:(NSURL *_Nullable)location
                  successHandler:(VRThemeGeneratePathSuccessHandler _Nullable)successHandler
                    errorHandler:(VRThemeErrorHandler _Nullable)errorHandler;
